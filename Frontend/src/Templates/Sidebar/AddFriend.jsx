...
export default function AddFriendCard({ isMobile }) {
  const { setIsCardOpen } = useContext(Store);
...
console.log("Search Result:", searchQuery);
  ...
  const handleAddFriend = () => {
       setTimeout(async () => {
        console.log("Adding friend with ID:", searchQuery);
      try {
        const response = await fetch(
  ...
  return (
    <div
      className={`w-full min-h-screen ${
        isMobile ? "absolute top-1/2 left-1/2" : "absolute top-1/2 left-2/3"
          ...
          <div className="bg-white rounded-xl shadow-lg p-4">
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-all duration-200 flex items-center gap-2 font-medium"
              >
          ...
          <div className="px-6 pb-6">
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <p className="text-sm text-blue-800">
                <span className="font-semibold">Tip:</span> Try searching for:
                sarahj, mchen, emmaw, janderson, or oliviam
              </p>
            </div>
          </div>
        )}
