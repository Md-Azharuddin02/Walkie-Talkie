...
import { TextField, Box, Typography, Container, Link, CircularProgress } from '@mui/material';
...
function Register() {
    ...
    const handleSendOTP = async () => {
        ...
        setIsLoading(true);
        try {
            const response = await axios.post('http://localhost:5803/api/auth/send-otp', {
                phoneNumber: phone
            });
            ...
            setErrors({ 
                submit: error.response?.data?.error || 'Failed to send OTP. Please try again.' 
            });
        } finally {
            setIsLoading(false);
        }
    ...
    const handleVerifyOTP = async () => {
        ...
        setIsLoading(true);
        try {
            const response = await axios.post('http://localhost:5803/api/auth/verify-otp', {
                phoneNumber: phone,
                otp: otpValue
            });
    ...
    return (
        <Box sx={styles.wrapper}>
            ...
            <Container component="main" maxWidth="xs" sx={{ position: 'relative', zIndex: 1 }}>
                <Box sx={styles.card}>
                    <Typography variant="h5" component="h1" sx={{ mb: 3, textAlign: 'center', color: '#1976d2' }}>
                        Phone Verification
                    </Typography>
                    ...
                    <Box sx={styles.linkContainer}>
                        ...
                        <Link
                            ...
                            className={!phone || isLoading || remainingTime > 0 ? 'disabled' : ''}
                            sx={styles.link}
                        >
                    ...
                    {isOtpSent && (
                        <Link
                            ...
                            sx={{ ...styles.link, display: 'block', textAlign: 'center', mt: 2 }}
                        >
                            Verify OTP
                        </Link>
                    )}
